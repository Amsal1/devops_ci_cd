void setBuildStatus(String message, String state) {
  step([
      ...
      reposSource: [$class: "ManuallyEnteredRepositorySource", url: "https://github.com/Amsal1/devops_ci_cd"],
	...
	stage('Deploying on Docker') {
		...
		steps {
			script {
				if (env.BRANCH_NAME == 'master'){
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://172.105.54.226:83/index.html` = 200 ];
				then
				...
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://172.105.54.226:82/index.html` = 200 ];
				then
				echo "Merge Successful"
				else
				echo "Merge Failed"
